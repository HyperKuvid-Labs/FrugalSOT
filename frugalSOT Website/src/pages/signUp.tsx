...
export default function SignUpPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        const resp = await axios.post("https://harish20205-frugalsot.hf.space/api/sign-up", {
          name,
          email,
          password,
        });
            ...
            <div className="bg-black/50 backdrop-blur-xl rounded-xl p-8 relative overflow-hidden">
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full py-3 bg-yellow-500 text-black rounded-md font-medium hover:bg-yellow-400 transition-colors flex items-center justify-center"
                >
                  Create Account
...
function FloatingInput({
  ...
  return (
    <div className="relative">
      {icon && (
        <span
          className={`absolute left-[0.75rem] top-[50%] -translate-y-[50%] ${
            isFocused || hasValue ? "text-yellow-400" : "text-gray-400"
          } transition-colors`}
        >
          {icon}
        </span>
